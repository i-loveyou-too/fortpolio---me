    ...
    <ScreenShell withBottomNav>
      <div className="-mx-5 -mt-7 min-h-screen bg-[radial-gradient(circle_at_50%_-5%,#D9F6FF_0,#EEF9FF_34%,#F8FBFF_68%)] px-5 pb-36 pt-10">
        ...
        <div className="mt-4 space-y-3">
          {active && (
            <StateCard
              title={active.attempt_no === 1 ? "최초 응시 중" : "답안 재입력 중"}
              desc="저장된 답안을 이어서 입력할 수 있습니다."
              tone="blue"
            />
          )}
          {!active && data.assignment.available_retake_approval_count > 0 && (
            <StateCard title="답안 재입력 열림" desc="모의고사 답안 재입력이 열렸습니다." tone="blue" />
          )}
          {!active && latest?.status === "submitted" && data.assignment.available_retake_approval_count === 0 && (
            <StateCard title="제출 완료" desc="답안 제출이 완료되었습니다. 선생님의 채점과 결과 공개를 기다려주세요." tone="orange" />
          )}
          {!active && latest?.status === "scored" && data.assignment.available_retake_approval_count === 0 && (
        ...
        <div className="fixed bottom-[calc(5.5rem+env(safe-area-inset-bottom))] left-1/2 z-30 w-full max-w-[430px] -translate-x-1/2 px-5 md:max-w-[760px] lg:max-w-[1180px] lg:px-6">
          {active ? (
            <Link href={`/student/sprint/exams/attempts/${active.id}`} className="block h-14 rounded-[20px] bg-[#2874E8] text-center text-base font-black leading-[3.5rem] text-white shadow-[0_16px_35px_rgba(40,116,232,0.28)]">답안 재입력 계속하기</Link>
          ) : data.assignment.available_retake_approval_count > 0 ? (
            <div className="space-y-2">
              ...
              {latest?.status === "scored" && (
                <Link href={`/student/sprint/exams/attempts/${latest.id}/result`} className="block h-12 rounded-[16px] bg-white border border-[#DCE4ED] text-center text-sm font-black leading-[3rem] text-[#2874E8]">결과 보기</Link>
              )}
            </div>
