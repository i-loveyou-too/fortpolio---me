...
def admin_create_sprint_exam_v2_retake_approval(
    ...
    try:
        check_existing = payload.reason is None
        return retake_service.create_retake_approval(db, _payload(payload), admin_id=_admin.id, check_existing=check_existing)
