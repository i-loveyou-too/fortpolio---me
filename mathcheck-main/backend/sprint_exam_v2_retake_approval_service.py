    ...
    try:
        ...
        if check_existing:
            base_attempts = [
                ...
                if a.retake_approval_id is None and a.status in ACTIVE_ATTEMPT_STATUSES
            ]
...
def cancel_retake_approval(db: Session, approval_id: int) -> dict[str, Any]:
    ...
    try:
        approval = get_retake_approval(db, approval_id, lock=True)
        used_attempt = _used_attempt(db, approval.id)
        computed = compute_approval_status(approval, used_attempt=used_attempt)
        if computed == "cancelled":
            return {"approval": _serialize_approval(db, approval, include_admin_fields=True)}
        if computed in {"used", "expired"}:
            if computed == "used":
                raise SprintExamV2RetakeApprovalConflictError("RETAKE_APPROVAL_NOT_CANCELLABLE", "Used retake approvals cannot be cancelled.")
            if computed == "expired" and used_attempt is not None:
                raise SprintExamV2RetakeApprovalConflictError("RETAKE_APPROVAL_NOT_CANCELLABLE", "Used retake approvals cannot be cancelled.")
        if used_attempt is not None:
            raise SprintExamV2RetakeApprovalConflictError("RETAKE_APPROVAL_NOT_CANCELLABLE", "Retake approvals with active attempts cannot be cancelled.")
