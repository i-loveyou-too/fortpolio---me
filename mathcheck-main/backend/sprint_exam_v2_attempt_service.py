...
def _validate_assignment_startable(assignment: models.SprintExamV2Assignment, *, now: datetime, skip_due_at_check: bool = False) -> None:
    ...
    if not skip_due_at_check and due_at is not None and now > due_at:
...
def start_attempt(db: Session, assignment_id: int, student_id: int) -> dict[str, Any]:
    ...
    try:
        ...
        if started is not None:
            _validate_assignment_startable(assignment, now=now, skip_due_at_check=started.retake_approval_id is not None)
            return {
        ...
        _validate_assignment_startable(assignment, now=now, skip_due_at_check=retake_approval is not None)
